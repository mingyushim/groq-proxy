...
const DEEP_HOLE_URL = "https://mabimobi.life/d/api/v1/main/deep-hole";
const WEBHOOK_URL = "https://discord.com/api/webhooks/1382217136667230218/mwewhH4pp6kOjvWGji_9ZfsTdFeVUmwD2T_tAjWNbV4CFCTdRpRpdj4-0JSmuL8tTNN7";
...
// 🔁 심층 구멍 상태 주기적 확인 및 알림
setInterval(async () => {
  try {
    const response = await axios.get(DEEP_HOLE_URL);
    const data = response.data;

    const duncan = data.find(item => item.server === "03"); // 던컨
    if (!duncan) return;

    const currentStatus = duncan.status; // "open" or "closed"
    ...
    if (currentStatus !== lastStatus) {
      ...
      console.log("🔔 디스코드 알림 전송됨:", isOpen ? "열림" : "닫힘");
    }
  } catch (error) {
    console.error("심층 상태 확인 실패:", error.response?.data || error.message);
  }
}, 30000); // 30초마다 체크
